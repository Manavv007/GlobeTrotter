...
const PlanTripPage = () => {
  ...
  // Custom trip autocomplete state
  const [customStartSuggestions, setCustomStartSuggestions] = useState([]);
  const [customEndSuggestions, setCustomEndSuggestions] = useState([]);
  const [showCustomStartSuggestions, setShowCustomStartSuggestions] = useState(false);
  const [showCustomEndSuggestions, setShowCustomEndSuggestions] = useState(false);
  ...
  // Handle custom trip place selection
  const selectCustomPlace = (place, field, setSuggestions, setShowSuggestions) => {
                ...
                <div className="relative">
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => {
                        handleCustomTripChange('startPlace', e.target.value);
                        searchPlaces(e.target.value, setCustomStartSuggestions, setShowCustomStartSuggestions);
                      }}
                      onFocus={() => {
                        if (customTripData.startPlace.length >= 2) {
                          setShowCustomStartSuggestions(true);
                        }
                      }}
                      placeholder="e.g., Mumbai, Maharashtra"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  ...
                  {/* Custom Start Place Suggestions */}
                  {showCustomStartSuggestions && customStartSuggestions.length > 0 && (
                    <div className="absolute z-20 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                      {customStartSuggestions.map((place, index) => (
                        <div
                          key={index}
                          onMouseDown={(e) => {
                            e.preventDefault();
                            selectCustomPlace(place, 'startPlace', setCustomStartSuggestions, setShowCustomStartSuggestions);
                          }}
                ...
                <div className="relative">
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => {
                        handleCustomTripChange('endPlace', e.target.value);
                        searchPlaces(e.target.value, setCustomEndSuggestions, setShowCustomEndSuggestions);
                      }}
                      onFocus={() => {
                        if (customTripData.endPlace.length >= 2) {
                          setShowCustomEndSuggestions(true);
                        }
                      }}
                      placeholder="e.g., Delhi, India"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  ...
                  {/* Custom End Place Suggestions */}
                  {showCustomEndSuggestions && customEndSuggestions.length > 0 && (
                    <div className="absolute z-20 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                      {customEndSuggestions.map((place, index) => (
                        <div
                          key={index}
                          onMouseDown={(e) => {
                            e.preventDefault();
                            selectCustomPlace(place, 'endPlace', setCustomEndSuggestions, setShowCustomEndSuggestions);
                          }}
                          className="px-3 py-2 hover:bg-gray-100 cursor-pointer border-b border-gray-100 last:border-b-0"
                        >
                          <div className="font-medium text-gray-900">{place.main_text}</div>
