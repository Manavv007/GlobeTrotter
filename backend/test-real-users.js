...
/**
 * Test Script for Multi-User Authentication using REAL users
...
// Test users (using your actual users)
const testUsers = [
  { email: 'test@example.com', password: 'password123' },
  { email: 'dp269211@gmail.com', password: 'password123' },
  { email: 'campuscreationsteam@gmail.com', password: 'password123' }
];

const activeSessions = [];

async function testMultiUserLogin() {
  console.log('🚀 Testing Multi-User Authentication with REAL Users');
  console.log('===================================================\n');
  ...
  try {
    ...
    const loginPromises = testUsers.map(async (user, index) => {
      try {
        ...
        if (error.response?.status === 401) {
          console.log(`   🔍 Invalid credentials - check password for ${user.email}`);
        }
    ...
    if (successfulLogins.length === 0) {
      console.log('❌ No users could log in. Please check your server and passwords.');
      console.log('💡 Make sure your server is running and passwords are correct.');
      return;
    }
